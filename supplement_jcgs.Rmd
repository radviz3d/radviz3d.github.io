...
# for ceramic sample data
glaze = brewer.pal(12,"Paired")[c(2,4,6,10)]
body = lighten(glaze, 0.6) # higher percetange -> ligher color
ceramic.palette = c(body[1],glaze[1],body[2],glaze[2],
                    body[3],glaze[3],body[4],glaze[4])
# for SARS-COV2 data
...
## Fig. S7: Ceramic Sample Datasets.
```{r}
...
<td align="center" valign="center">
```{r}
modradviz2d(x = df.17[,-c(1,2)][ll], cl = factor(df.17$era), modify = F, 
            coord.labels = colnames(df.17[,-c(1,2)]), class.labels = NULL,
            palette = ceramic.palette, pch = c(16,17)[factor(df.17$mf)],
            opt.axis.order = F)
```
<br />
RadViz2D
...
<td align="center" valign="center">
```{r}
res1 = viz2d_3d(df.17[,-c(1,2)][ll])
res1 = data.frame(res1, as.factor(df.17$era))
colnames(res1) = c("x","y","z","class")
# open 3d window
# resize window
par3d(windowRect = c(100, 100, 612, 612))
# plot
pch3d(x = res1, y = NULL, z = NULL, xlab = "", ylab="", zlab="",axes = F,cex = 0.5,
      pch = c(16,17)[factor(df.17$mf)], size = 0.95, color = ceramic.palette)
...
<td align="center" valign="center">
```{r}
radialvis3d(data = df.17[,-c(1,2)][ll], cl = factor(df.17$era), domrp = F, doGtrans = F, 
            lwd = 3, alpha = 0.025, color = ceramic.palette, with.class.labels = FALSE,
            ret.trans = F, pch =  c(16,17)[factor(df.17$mf)], point.cex = 0.25)
...
## Use hclust to find the order of coordinates for RadViz displays
#xx  <-  t(scale(asin(sqrt(covid19.variants[,-c(1:3)]))))
#x.hcl  <- hclust(dist(xx), method = "ward.D2")
#plot(x.hcl)
...
covid19.variants$group <- factor(covid19.variants$group, levels = unique(covid19.variants$group))
```
...
<td align="center" valign="center">
```{r}
modradviz2d(x = covid19.variants[,-c(1:3)][ll], 
            cl = factor(covid19.variants$region), modify = F, ret.proj = T,
            coord.labels = colnames(covid19.variants[,-c(1:3)][ll]), 
            class.labels = NULL, palette = kols, 
            pch = letters[covid19.variants$group], opt.axis.order = F) -> xproj
new.dat <- data.frame(xproj, group = covid19.variants$group, 
                      region = covid19.variants$region)
for (i in 1:10) {
  dat <- new.dat[new.dat$region==i,]
  for (j in 1:nlevels(covid19.variants$group))
...
<td align="center" valign="center">
```{r}
res1 = viz2d_3d(covid19.variants[,-c(1:3)][ll])
res1 = data.frame(res1, as.factor(covid19.variants$region))
colnames(res1) = c("x","y","z","class")
# open 3d window
# resize window
par3d(windowRect = c(100, 100, 612, 612))
...
pch3d(x = res1, y = NULL, z = NULL, xlab = "", ylab="", zlab="",axes = F,
      pch = letters[covid19.variants$group],size = 0.95, color = kols)
new.dat <- data.frame(res1, group = covid19.variants$group, 
                      region = covid19.variants$region)
for (i in 1:10) {
    dat <- new.dat[new.dat$region==i,]
    rgl::rgl.texts(x = dat[1, 1] - .025, y = dat[1, 2] + .025, z = dat[1, 3] - .025, 
                   text = as.character(i), cex = 3, color = kols[i])
    for (j in 2:nlevels(covid19.variants$group))
        rgl.dashed.lines(start.pt = dat[j-1,1:3], end.pt = dat[j,1:3], dash.length = 0.01, 
                         color = kols[i], add = T, lwd = 3)
}
box3d(color = "black",lwd = 1)
rglwidget()
rgl.close()
...
<td align="center" valign="center">
```{r}
radviz3d::radialvis3d(data = covid19.variants[,-c(1:3)][ll], 
                      cl = factor(covid19.variants$region), domrp = F, 
                      doGtrans = F, lwd = 3, alpha = 0.025, color = kols, 
                      with.class.labels = FALSE, shininess = 128, specular = 'black', 
                      ret.trans = TRUE, pch = letters[covid19.variants$group],  
                      coord.cex = 2,point.cex = 2)-> dat.trns

new.dat <- data.frame(dat.trns, group = covid19.variants$group, 
                      region = covid19.variants$region)
for (i in 1:10) {
    dat <- new.dat[new.dat$region==i,]
    rgl::rgl.texts(x = dat[1, 1] - .025, y = dat[1, 2] + .025, z = dat[1, 3] - .025, 
