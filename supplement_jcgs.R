...
kols2 <- c(RColorBrewer::brewer.pal(name = "Dark2", n = 8),
           ...
           RColorBrewer::brewer.pal(name = "Blues", n = 9)[5])
kols <- kols2

# Plot dashed lines 
rgl.dashed.lines <- function(start.pt, end.pt, dash.length, ...)
{
    curr.pt <- start.pt
    direction <- (end.pt - start.pt)/sqrt(sum((end.pt - start.pt)^2))
    next.pt  <- curr.pt + dash.length * direction

    magnitude <- sqrt(sum((end.pt - start.pt)^2))
    k <- magnitude/dash.length
    j <- 0
    while (j < k) {
        j <- j + 1
        rgl::rgl.lines(x = rbind(curr.pt,next.pt), ...)
        curr.pt  <- next.pt + dash.length * direction
        remaining.length <- sqrt(sum((end.pt-curr.pt)^2))
        next.pt <- curr.pt + min(dash.length, remaining.length) * direction
        j  <- j + 1
    }
}
...
ll <- c()
ll[c(4,9,1,6,3)] <- c(6, 17, 14, 9, 2)
ll[c(2,5,7,10,8)] <- c(5, 13, 1, 10, 4)
...
modradviz2d(x = df[,-c(1:3)][ll], 
            ...
            class.labels = NULL, palette = kols, 
            pch = letters[df$group], opt.axis.order = F) -> xproj
new.dat <- data.frame(xproj, group = df$group, 
                      region = df$region)
for (i in 1:10) {
...
# plot
pch3d(x = res1[,1], y = res1[,2], z = res1[,3], xlab = "", ylab="", zlab="",
      axes = F, pch = letters[df$group], cex = 1, color = kols)
new.dat <- data.frame(res1, group = df$group, 
                      region = df$region)
for (i in 1:10) {
    dat <- new.dat[new.dat$region==i,]
    rgl::rgl.texts(x = dat[1, 1] - .025, y = dat[1, 2] + .025, z = dat[1, 3] - .025, 
                   text = as.character(i), cex = 1.2, color = kols[i])
    for (j in 2:nlevels(df$group))
        rgl.dashed.lines(start.pt = dat[j-1,1:3], end.pt = dat[j,1:3], dash.length = 0.01, 
                         color = kols[i], add = T, lwd = 1)
}
box3d(color = "black", lwd = 1)
rgl.viewpoint(zoom = 0.6)
rglwidget()
rgl.close()
...
radviz3d::radialvis3d(data = df[,-c(1:3)][ll], 
                      ...
                      with.class.labels = FALSE, shininess = 128, specular = 'black', 
                      ret.trans = TRUE, pch = letters[df$group],  
                      coord.cex = 1.5,point.cex = 1.15)-> dat.trns

new.dat <- data.frame(dat.trns, group = df$group, 
                      region = df$region)
for (i in 1:10) {
    dat <- new.dat[new.dat$region==i,]
    rgl::rgl.texts(x = dat[1, 1] - .025, y = dat[1, 2] + .025, z = dat[1, 3] - .025, 
                   text = as.character(i), cex = 1.25, color = kols[i])
    for (j in 2:nlevels(df$group))
        rgl.dashed.lines(start.pt = dat[j-1,1:3], end.pt = dat[j,1:3], dash.length = 0.01, 
                         color = kols[i], add = T, lwd = 1.25)
}
rgl.viewpoint(zoom = 0.6)
